...
### 3.3 SLA-Escalation Queries

SLA Escalation is used to identify tickets that have remained unresolved beyond a defined time limit. Since closed tickets are not relevant for SLA monitoring, a partial index on only unresolved tickets (`open`, `pending` statuses) was created. This will reduce the index and ensures the SLA Escalation Queries do not waste resources. By including `createdAt` in the index will allow the database to filter out the results in the given time range.
